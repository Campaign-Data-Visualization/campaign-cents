...
var updateContribs = function() {
  ...
  db.doQuery("delete from koch_contribs")
    .then(function() { return db.doQuery("insert into koch_contribs select cid, ultorg, pacid, date, amount, '', 'pac_to_can', fecrecno, 'f', cycle, null, null from IFG_PAC2Cand_Data"); })
    .then(function() { return db.doQuery("insert into koch_contribs select recipid, if(orgname != '', orgname, if(ultorg != '', ultorg, fecoccemp)), contribid, date, amount, '', 'individual', fectransid, 'f', cycle, null, null from IFG_IndivData"); })
    .then(function() { return db.doQuery("insert into koch_contribs select cid, if(ultorg != '', ultorg, pacshort), '', date, amount, '', 'outside_cand', id, if(crpsuppopp = 'S', 'f', 'a'), cycle, null, null from IFG_Outside_Cand_Expends"); })
    ...
    .then(function() { return db.doQuery("update koch_contribs a join koch_orgs b on donor_name_orig = org_name set a.koch_tier = b.tier, a.donor_name = b.org_alias "); })
